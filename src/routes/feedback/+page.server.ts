...
/**
 * Per-IP flood guard, same approach as replay uploads: single always-on
 * machine, so in-memory is fine (and `fly apps restart` clears it).
 *
...
export const actions: Actions = {
	default: async ({ request, getClientAddress }) => {
		...
		// so an Atlas hiccup costs a retry rather than the whole message.
		try {
